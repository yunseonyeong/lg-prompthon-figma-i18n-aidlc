...
// Step 8은 LLM을 쓰지 않는다. Step 6이 쓴 구조 파일을 미리보기와 같은 규칙으로 JSX로 옮긴다.
import { generateComponentsFromStructure } from '../codegen/figma-to-tsx.js';
...
export async function runPipeline(fileKey?: string): Promise<void> {
  ...
  // Step 8: React 컴포넌트 코드 생성 (Figma 구조 → JSX, LLM 호출 없음)
  //
  // Step 6이 방금 쓴 figma-structure.json을 그대로 입력으로 쓴다. 미리보기(Step 6 화면)와
  // 같은 파일·같은 변환 규칙이라 화면과 코드가 어긋나지 않는다.
  console.log('⚛️  Step 8: Figma 구조 → React 컴포넌트 코드 생성...');
  const componentResult = generateComponentsFromStructure({
    structurePath: path.resolve(CONFIG.outputDir, '..', 'figma-structure.json'),
    componentsMapPath: path.resolve(CONFIG.outputDir, '..', 'components-map.json'),
  });
