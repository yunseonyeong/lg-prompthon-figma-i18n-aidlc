...
- 번역하는 서비스를 선택하고 설명을 지정하게 하여 해당 서비스에 맞게 번역되도록 유도한다.

### FR-03: i18n Key 생성
- 네이밍 규칙: `{domain}.{feature}.{role}.{identifier}` # 수정필요
...
- 출력 형식: 계층적 JSON 구조 (프론트 개발자가 파일을 바로 사용가능하다)
- 모든 locale 파일의 key 구조가 동일해야 한다.
- 확장성을 고려해서 추가 지원 언어가 있다.

### FR-05: React 컴포넌트 코드 생성
- EXAONE API를 통해 컴포넌트 코드를 생성하고 다국어를 자동으로 적용시킨다.
...
- QA와 같은 검증원에게 각 문구의 출력위치를 확인할 수 있는 경로를 제공한다.
- 각 번역에 대해 해외 현지 법인의 감수를 여부를 확인할 수 있다.

### FR-08: 데모 웹서버
- 언어 선택 드롭박스 → 해당 언어로 컴포넌트 렌더링
- 실시간 언어 전환 데모 지원
- 모든 요구사항의 동작을 데모에서 표현된다
- 일련의 진행 프로세스를 수행하여 결과를 도출하면 절약된 예산을 표시하고 현재까지 해당 플랫폼을 통해 절약된 예산을 누적 계산한다.
...
### NFR-01: 성능
- 100개 텍스트 기준 전체 파이프라인(추출→번역→출력) 5분 이내
