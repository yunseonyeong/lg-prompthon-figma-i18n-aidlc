...
| Workspace | Business Site 하위 사이트 단위 | 워크스페이스 | ワークスペース | 工作区 |
| Group | 비즈니스 사이트/유저의 묶음 단위 | 그룹 | グループ | 组 |
| Device | 연동된 디바이스 | 디바이스 | デバイス | 设备 |
| User | 콘솔 사용자 | 사용자 | ユーザー | 用户 |
| Console | 관리 콘솔 | 콘솔 | コンソール | 管理控制台 |
...
| Client Secret | 클라이언트 시크릿 | 클라이언트 비밀키 | クライアントシークレット | 客户端密钥 |
...
| English | Context | ko | ja | zh-CN |
|---------|---------|-----|-----|-------|
| Vertical Type | **업종/산업 분야** 분류 | 버티컬 타입 | 業種 | 行业类型 |
| Business Type | 사업 유형 | 비즈니스 타입 | ビジネスタイプ | 业务类型 |
| System Integrator | SI 사업자 | 시스템 통합 업체 | システムインテグレーター | 系统集成商 |
...
| English | Context | ko | ja | zh-CN |
|---------|---------|-----|-----|-------|
| Dashboard | 대시보드 메뉴 | 대시보드 | ダッシュボード | 管理界面 |
| Space | 공간 관리 메뉴 | 스페이스 | スペース | 空间 |
| Content | 재생 콘텐츠 | 콘텐츠 | コンテンツ | 多媒体 |
| Schedule | 재생/제어 스케줄 | 스케줄 | スケジュール | 日程 |
| Content Schedule | 콘텐츠 재생 스케줄 | 콘텐츠 스케줄 | コンテンツスケジュール | 多媒体日程 |
| Control Schedule | 디바이스 제어 스케줄 | 제어 스케줄 | 制御スケジュール | 控制日程 |
...
7. 단수/복수는 한국어에서 구분하지 않는다. `Device`/`Devices` 모두 `디바이스`.
